...
class LLMDiary:
    ...
    async def ai_llm_label_model_run(self, input_data: str, config: dict) -> str:
        ...
        prompt = ChatPromptTemplate.from_template("""[Instruction]
                                                    ...
                                                    6. "주요라벨"은 "원본"의 전체를 확인해서 해당되는 것을 하나 출력하세요.
                                                    7. "마일스톤"은 "원본"을 확인해서 해당되는 것을 추출하세요.
                                                    8. [예시]의 모든 항목은 누락 없이 전부 화면에 출력되어야 하며, 원문에 내용이 없거나 추출할 수 없는 항목은 오직 '없음'이라는 두 글자만을 채워서 깔끔하게 출력하세요.
