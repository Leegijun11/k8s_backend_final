    ...
    try:
        ...
        for idx, label_dict in enumerate(label_list):
            ...
            for key in merged_labels.keys():
                ...
                if val and val != "없음":
                    merged_labels[key].append(val)

        structured_text = "\n".join(raw_sentences)

        if merged_labels["부모감정"]:
            p_emotion = ", ".join(dict.fromkeys(merged_labels["부모감정"]))
        else:
            p_emotion = "평온하다"

        if merged_labels["아이감정"]:
            b_emotion = ", ".join(dict.fromkeys(merged_labels["아이감정"]))
        else:
            b_emotion = "기쁘다"
        ...
        labels = {
            "원본": input,
            "핵심어": ", ".join(dict.fromkeys(merged_labels["핵심어"])) if merged_labels["핵심어"] else "없음",
            "부모감정": p_emotion,
            "아이감정": b_emotion,
            "식사": ", ".join(merged_labels["식사"]) if merged_labels["식사"] else "없음",
            "배변": ", ".join(merged_labels["배변"]) if merged_labels["배변"] else "없음",
            "수면": ", ".join(merged_labels["수면"]) if merged_labels["수면"] else "없음",
            "시간": ", ".join(dict.fromkeys(merged_labels["시간"])) if merged_labels["시간"] else "없음",
            "체온": ", ".join(dict.fromkeys(merged_labels["체온"])) if merged_labels["체온"] else "없음",
